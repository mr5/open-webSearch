...
- Web search using multi-engine results
    ...
    - x / twitter (external browser only)
...
| `DEFAULT_SEARCH_ENGINE` | `bing`                  | `bing`, `duckduckgo`, `exa`, `brave`, `baidu`, `csdn`, `linuxdo`, `juejin`, `startpage`, `sogou`, `hackernews`, `jd`, `taobao`, `alibaba`, `xiaohongshu`, `zhihu`, `x` | Default search engine |
...
```typescript
{
  ...
  "engines": string[],    // Optional: Engines to use (bing,baidu,linuxdo,csdn,duckduckgo,exa,brave,juejin,startpage,sogou,hackernews,jd,taobao,alibaba,xiaohongshu,zhihu,x) default runtime-configured engine
  "searchMode": string    // Optional: request, auto, or playwright (affects Bing; protected engines below are external-browser-only)
}
```
...
4. **Search Engine Configuration**:
   ...
   - Supported engines: bing, duckduckgo, exa, brave, baidu, csdn, linuxdo, juejin, startpage, sogou, hackernews, jd, taobao, alibaba (1688), xiaohongshu, zhihu, x (alias: twitter)
   - JD, Taobao, Alibaba/1688, Xiaohongshu, Zhihu, and X require `BROWSER_BACKEND=external`. Search and `fetchWebContent` for these sites never use direct HTTP or local Chromium. The external worker keeps authenticated per-site search tabs open for login/CAPTCHA handling.
