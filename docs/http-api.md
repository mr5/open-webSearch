...
- `searchMode` affects Bing. `jd`, `taobao`, `alibaba` (1688), `xiaohongshu`, `zhihu`, and `x` (also accepted as `twitter`) always require `BROWSER_BACKEND=external`, even when `searchMode=request`; local Chromium and direct HTTP are disabled for them.
